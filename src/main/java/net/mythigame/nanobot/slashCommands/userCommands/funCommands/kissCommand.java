...
public class kissCommand implements SlashCommand {
    ...
    public void performCommand(SlashCommandEvent event, Member member, TextChannel channel) {
        ...
        if(option != null){
            ...
            event.reply(member.getAsMention() + " vient d'embrasser " + target.getAsMention()).queue();
        }
        event.reply("Voici :").queue(e -> e.deleteOriginal().queue());
        channel.sendMessage(CONFIGURATION.getString("kissGifAPI", "Lien du répertoire de gifs kiss") + randomGif + ".gif").queue();
    }
}
