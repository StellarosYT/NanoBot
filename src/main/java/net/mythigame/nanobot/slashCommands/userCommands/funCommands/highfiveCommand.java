...
public class highfiveCommand implements SlashCommand {
    @Override
    public void performCommand(SlashCommandEvent event, Member member, TextChannel channel) {
        ...
        if(option != null){
            ...
            event.reply(member.getAsMention() + " tape m'en cinq " + target.getAsMention()).queue();
        }
        event.reply("Voici :").queue(e -> e.deleteOriginal().queue());
        channel.sendMessage(CONFIGURATION.getString("highfiveGifAPI", "Lien du répertoire de gifs highfive") + randomGif + ".gif").queue();
    }
}
