...
public class slapCommand implements SlashCommand {
    ...
    public void performCommand(SlashCommandEvent event, Member member, TextChannel channel) {
        ...
        if(option != null){
            ...
            event.reply(member.getAsMention() + " vient de frapper " + target.getAsMention()).queue();
        }
        event.reply("Voici :").queue(e -> e.deleteOriginal().queue());
        channel.sendMessage(CONFIGURATION.getString("slapGifAPI", "Lien du répertoire de gifs slap") + randomGif + ".gif").queue();
    }
}
